...
`make run-staging`
### В докере на 80 порту живет nginx, так что дока находиться по адресу http://localhost/api/public/docs
...
### Дока доступна по адресу
http://localhost:8889/api/public/docs

### Для конфигурированием аппой используется модуль `core/config.py`, для Docker через переменные окружения `.env.example` 

___

## Postmortens:
### 1) Для простоты я не стал реализовывать полноценную механику инвалидации кэша + сборка кэша для получения всех контактов, от этого больше вреда, чем пользы(битый или тухлый кэш на L7/CDN, оч вероятно заафектит юзеров, в таком случае придется сбрасывать вручную на уровне CDN)
